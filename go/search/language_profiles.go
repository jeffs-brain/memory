...
// languageProfiles contains character bigram frequency vectors for
// language detection. Each profile is derived from representative
// corpora and normalized. The profiles are intentionally compact — only
// the top-frequency bigrams per language are retained to keep memory
// usage bounded while providing sufficient discriminative power.
//
// Languages without profiles (CJK) are detected via Unicode script
// checks in the CJK tokenizer, not via this frequency-based method.
var languageProfiles = map[string]map[string]float64{
	"en": {
