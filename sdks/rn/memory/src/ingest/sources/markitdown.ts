...
const fileExtension = (filename: string | undefined): string | undefined => {
  ...
  return ext === undefined ? undefined : `.${ext.toLowerCase()}`
}
...
export const loadMarkitdownFile = async (
  ...
  const blobBytes: ArrayBuffer =
    bytes instanceof Uint8Array ? new Uint8Array(bytes).buffer : bytes.slice(0)
  form.set('file', new Blob([blobBytes], { type: mime }), filename)
