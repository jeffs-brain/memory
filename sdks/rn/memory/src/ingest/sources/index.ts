...
import { type LoadUrlOptions, fetchUrlSource, htmlToMarkdown, loadUrl } from './url.js'
...
export { htmlToMarkdown, loadUrl, fetchUrlSource } from './url.js'
...
export const loadSource = async (
  ...
  if (input.kind === 'url') {
    const urlOptions: LoadUrlOptions = {
      ...(opts.title !== undefined ? { title: opts.title } : {}),
      ...(opts.fetch !== undefined ? { fetch: opts.fetch } : {}),
    }
    const fetched = await fetchUrlSource(input.url, urlOptions)
    const resolvedTitle = opts.title ?? fetched.title
    const loaded = await loadSource(
      {
        kind: 'bytes',
        bytes: new TextEncoder().encode(fetched.content),
  ...
  const subOptions: SourceLoadOptions = {
    ...(opts.title !== undefined ? { title: opts.title } : {}),
    ...(filename !== undefined ? { filename } : {}),
    ...(mime !== undefined ? { mime } : {}),
  }
  ...
  switch (kind) {
    case 'markdown':
      return loadMarkdown(bytes, subOptions)
    case 'html': {
      const body = htmlToMarkdown(new TextDecoder('utf-8').decode(bytes))
    ...
    case 'pdf':
      return loadPdf(bytes, subOptions)
    case 'json-transcript':
      return loadJsonTranscript(bytes, subOptions)
    case 'text':
      return loadText(bytes, subOptions)
    case 'url':
      throw new Error('loadSource: URL kind requires input.kind === "url"')
    case 'binary':
      return {
        content: new TextDecoder('utf-8').decode(bytes),
        mime: subOptions.mime ?? 'application/octet-stream',
        ...(opts.title !== undefined ? { title: opts.title } : {}),
        meta: { binary: true },
      }
  }
