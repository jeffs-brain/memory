...
const path = (value: string): Path => value as Path
...
const makeStub = (): Store & {
  ...
  const stub: Store = {
    ...
    batch: async (_: BatchOptions, fn: (batch: Batch) => Promise<void>) => {
      calls.push({ method: 'batch', args: [] })
      const inner: Batch = {
        read: record('batch.read', 'x'),
...
describe('withAccessControl', () => {
  it('throws ForbiddenError and never hits the store on denied read', async () => {
    ...
    expect(inner.calls.find((call) => call.method === 'read')).toBeUndefined()
  })

  it('throws ForbiddenError and never hits the store on denied write', async () => {
    const inner = makeStub()
    const wrapped = withAccessControl(inner, denyAll, alice, { resource: brainResource })
    await expect(wrapped.write(path('foo'), 'x')).rejects.toBeInstanceOf(ForbiddenError)
    expect(inner.calls.find((call) => call.method === 'write')).toBeUndefined()
  })
  ...
  it('calls check with the correct subject/action/resource tuple', async () => {
    ...
    const wrapped = withAccessControl(inner, acl, alice, {
      resolveResource: (resolvedPath) => ({ type: 'document', id: resolvedPath }),
    })
  ...
  it('guards every op inside a batch', async () => {
    ...
    const wrapped = withAccessControl(inner, denyAll, alice, { resource: brainResource })
    await expect(
      wrapped.batch({ reason: 'test' }, async (batch) => {
        await batch.read(path('foo'))
      }),
    ).rejects.toBeInstanceOf(ForbiddenError)
    expect(inner.calls.find((call) => call.method === 'batch.read')).toBeUndefined()
  })

  it('preserves the wrapped store root', () => {
    const inner = makeStub()
    const wrapped = withAccessControl(inner, allowAll, alice, { resource: brainResource })
    expect(wrapped.root).toBe('/brains/demo')
  })
