 ...
 * authorise reads and writes against a `Store`. Ships with an in-process
 * RBAC adapter and a `Store` wrapper that enforces checks on the mobile
 * filesystem surface.
 */
 ...
 * optional: `write`/`read` because some providers (for example, a stateless
 * policy evaluator) do not persist tuples, and `close` because providers
 * that own no transport state have nothing to release.
 */
export type AccessControlProvider = {
   ...
   * handles, and so on). Implementations that own no resources may omit
   * this method.
   */
  close?(): Promise<void>
}
