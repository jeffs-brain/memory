...
export const createQuery = (deps: QueryDeps) => {
  ...
  return async (question: string, opts: QueryOptions = {}): Promise<QueryResult> => {
    ...
    try {
      ...
      return {
        answer: buildFallbackAnswer(provider, ranked.articles, error),
        sourcePaths,
        searchHits: ranked.searchHits,
      }
    }
...
const normaliseMaxSources = (maxSources: number | undefined): number => {
  ...
  if (rounded <= 0 || rounded > MAX_SOURCES_CAP) return DEFAULT_MAX_SOURCES
  return rounded
}
...
const hydrateRetrievedArticles = async (
  ...
  const hydrated = await Promise.all(
    Array.from(bestByPath.entries()).map(async ([path, metadata]) => {
      ...
      return {
        path: path as Path,
        title:
          parsed.frontmatter.title.trim() !== '' ? parsed.frontmatter.title.trim() : metadata.title,
  ...
  const ranked = hydrated.filter((article): article is QueryArticle => article !== undefined)
...
const orderForPrompt = (
  ...
  ordered.sort((left, right) => {
    const dateDiff = left.modified.getTime() - right.modified.getTime()
    if (sort === 'recency') {
...
const trimBody = (body: string): string => {
  if (body.length <= QUERY_MAX_BODY_CHARS) return body
