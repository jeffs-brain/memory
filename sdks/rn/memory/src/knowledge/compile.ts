...
export const createCompile = (deps: CompileDeps) => {
  ...
  return async (opts: CompileOptions = {}): Promise<CompileResult> => {
    ...
    for (const article of plan.newArticles) {
      ...
      if ((await readWikiArticle(store, existingWikiPath)) !== undefined) {
      ...
      const response = await provider.complete({
        ...
        messages: [{ role: 'user', content: buildCreatePrompt(article, sourceBodies) }],
        jsonMode: true,
        temperature: 0.2,
        maxTokens: 4096,
      })
      const parsed = parseWrite(response.content)
      ...
      pendingWrites.push({
        ...
        body: serialiseFrontmatter(frontmatter, parsed.body),
        kind: 'compile.write',
        title: article.title,
    ...
    for (const update of plan.updates) {
      await queueArticleRewrite(update, 'update')
    }
    for (const crossReference of plan.crossReferences) {
      await queueArticleRewrite(crossReference, 'crossref')
    }
    ...
    for (const rewrite of pendingArticleRewrites.values()) {
      ...
      pendingWrites.push({
        path: rewrite.path,
        body: serialiseFrontmatter(frontmatter, parsed.body || rewrite.existing.body),
    ...
    await store.batch({ reason: 'compile' }, async (batch) => {
      ...
      for (const write of pendingWrites) {
        await batch.write(write.path, write.body)
      ...
      for (const source of pendingProcessedSources) {
        ...
        await batch.write(
          processedMarkerPath(source.sourceId),
          serialiseProcessedMarker({
...
const listPendingIngestedSources = async (store: Store): Promise<readonly IngestedSource[]> => {
  ...
  for (const entry of entries) {
    ...
    if (pathUnder(entry.path, RAW_DOCUMENTS_ARCHIVE_PREFIX, true)) continue
    if (!entry.path.endsWith('.md')) continue

    const sourceId = entry.path.slice(RAW_DOCUMENTS_PREFIX.length + 1, -'.md'.length)
    const content = await store.read(entry.path)
    const marker = await readProcessedMarker(store, sourceId)
    if (isProcessedSourceContent(marker, entry.path, content)) continue
    sources.push({
      sourceId,
...
const listWikiIndex = async (store: Store): Promise<readonly WikiIndexArticle[]> => {
  ...
  const entries = await store.list(prefix, { recursive: true })
  const output: WikiIndexArticle[] = []
  for (const entry of entries) {
    if (entry.isDir) continue
    if (!pathUnder(entry.path, WIKI_PREFIX, true)) continue
    if (!entry.path.endsWith('.md')) continue

    const parsed = parseFrontmatter(await store.read(entry.path))
    output.push({
      path: entry.path,
...
const readWikiArticle = async (
  ...
  if (!(await store.exists(fullPath).catch(() => false))) return undefined
  const parsed = parseFrontmatter(await store.read(fullPath))
  return {
...
const buildPlanPrompt = (
  ...
  for (const source of ingested) {
    const snippet =
      source.body.length > 2000 ? `${source.body.slice(0, 2000)}\n[...truncated]` : source.body
...
const buildCreatePrompt = (
  ...
  for (let index = 0; index < sourceBodies.length; index += 1) {
    parts.push(`### ${article.sourceHashes[index] ?? ''}`, sourceBodies[index] ?? '', '')
  }
  return parts.join('\n')
}
...
const buildUpdatePrompt = (
  ...
  for (let index = 0; index < sourceBodies.length; index += 1) {
    parts.push(`### source-${index + 1}`, sourceBodies[index] ?? '', '')
  }
  return parts.join('\n')
}
...
const parseCreateList = (raw: RawPlan, max: number): CompilePlanArticle[] => {
  ...
  for (const article of source) {
    ...
    const hashes = article.source_hashes ?? article.sourceHashes ?? []
...
const parseUpdateList = (raw: RawPlan, max: number): CompilePlanUpdate[] => {
  ...
  for (const update of raw.updates ?? []) {
    ...
    const path = normaliseWikiRelativePath(update.path ?? '')
    ...
    updates.push({
      path,
      reason,
      sourceHashes: parseSourceIds(update.source_hashes ?? update.sourceHashes ?? []),
    })
  }
  return updates
}
...
const parseCrossReferenceList = (raw: RawPlan, max: number): CompilePlanCrossReference[] => {
  ...
  for (const crossReference of source) {
    if (crossReferences.length >= max) break
    const path = normaliseWikiRelativePath(crossReference.path ?? '')
    const reason = typeof crossReference.reason === 'string' ? crossReference.reason.trim() : ''
    if (path === '' || reason === '' || paths.has(path)) continue
    paths.add(path)
    crossReferences.push({
      path,
      reason,
      sourceHashes: parseSourceIds(
        crossReference.source_hashes ?? crossReference.sourceHashes ?? [],
      ),
    })
  }
...
const normaliseSlug = (value: string): string => tryNormaliseKnowledgeArticleStem(value) ?? ''

const normaliseWikiRelativePath = (value: string): string =>
  tryNormaliseWikiRelativeArticlePath(value) ?? ''
