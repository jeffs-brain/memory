import { type Batch, type Path, type Store, pathUnder } from '../store/index.js'
        ...
        for (const action of plan.actions) {
          if (action.kind === 'rehydrate_stub') {
            ...
            for (const sourcePath of action.sourcePaths) {
              reopenedSources.add(sourcePath)
            }
            applied.push(action)
            continue
          }
          ...
          for (const archivePath of action.archivePaths) {
            ...
            await batch.write(archivePath, serialiseFrontmatter(nextFrontmatter, nextBody))
...
const buildDuplicateArchiveActions = async (input: {
  ...
  for (const issue of input.report.issues) {
    ...
    for (const relatedPath of issue.details?.relatedPaths ?? []) {
      group.add(relatedPath)
    }
