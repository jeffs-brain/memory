 ...
 * on-disk layout stays identical between implementations.
 */

import { type Path, joinPath, validatePathSegment } from '../store/path.js'
...
export const scopeTopic = (scope: Scope, actorId: string, filename: string): Path => {
  ...
  return joinPath(scopePrefix(scope, actorId), safeName)
}
...
export const reflectionPath = (sessionId: string): Path =>
  joinPath(REFLECTIONS_PREFIX, `${sanitiseFilename(`${sessionId}.md`)}`)
