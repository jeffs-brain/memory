...
import { type Path, joinPath } from '../store/path.js'
...
export class StoreBackedProceduralStore implements ProceduralStore {
  ...
  async persist(args: PersistProceduralRecordsArgs): Promise<readonly StoredProceduralRecord[]> {
    ...
    await this.store.batch({ reason: persistReason(args.reason) }, async (batch) => {
      for (const record of stored) {
        await batch.write(record.path, Buffer.from(record.content, 'utf8'))
      }
    })
  ...
  private async scan(
    ...
    if (!(await this.prefixExists(prefix))) {
      return []
    }

    let entries: readonly { path: Path; isDir: boolean; modTime: Date }[]
    try {
    ...
    for (const entry of entries) {
      if (entry.isDir || !entry.path.endsWith('.md')) {
        continue
      }

      const record = await this.readStoredRecord(entry.path, entry.modTime)
      if (record === undefined || !matchesProceduralFilters(record, args)) {
        continue
      }
  ...
  private async readStoredRecord(
    ...
    let raw: Buffer
    try {
      raw = await this.store.read(path)
    } catch (err) {
      if (err instanceof ErrNotFound) {
        return undefined
      }
    ...
    if ((frontmatter.type ?? '').trim() !== 'procedural') {
      return undefined
    }
    ...
    if (tier === undefined || outcome === undefined || name === '') {
      return undefined
    }
...
const selectListPrefix = (actorId: string | undefined, sessionId: string | undefined): Path => {
  ...
  if (actor === undefined) {
    return PROCEDURAL_RECORDS_PREFIX
  }
...
const augmentProceduralNote = (
  ...
  const merged = [...lines.slice(0, closeIndex), ...extra, ...lines.slice(closeIndex)]
  return `${merged.join('\n').trimEnd()}\n`
}

const parseProceduralBody = (
  body: string,
): { taskContext: string; toolCalls: readonly string[] } => {
  ...
  const taskContext = contextLines.join('\n').trim()
  const toolCalls = collapseWhitespace(toolLines.join(' '))
    .split(/\s*->\s*/g)
    .map((value) => value.trim())
    .filter((value) => value !== '')

  return { taskContext, toolCalls }
}
...
const matchesProceduralFilters = (
  ...
  if (!matchesSelection(record.tier, args.tier)) {
    return false
  }
  if (!matchesSelection(record.outcome, args.outcome)) {
    return false
  }
  if (!matchesSelection(record.name, args.name)) {
    return false
  }
  ...
  if (tags.length > 0) {
    ...
    for (const tag of tags) {
      if (!recordTags.has(normaliseSearchText(tag))) {
        return false
      }
    }
  }
  ...
  if (since !== undefined && timestampValue(record.observedAt) < since) {
    return false
  }

  const until = parseTimestamp(args.until)
  if (until !== undefined && timestampValue(record.observedAt) > until) {
    return false
  }
...
const scoreProceduralRecord = (record: StoredProceduralRecord, query: string): number => {
  ...
  if (tags.some((tag) => tag.includes(phrase))) {
    score += 10
  }
  if (context.includes(phrase)) {
    score += 8
  }
  if (toolCalls.some((toolCall) => toolCall.includes(phrase))) {
    score += 8
  }
  if (record.tier === phrase || record.outcome === phrase) {
    score += 4
  }
  ...
  for (const token of tokens) {
    ...
    if (toolCalls.some((toolCall) => toolCall.includes(token))) {
      score += 3
    }
    if (context.includes(token)) {
      score += 2
    }
    if (record.tier === token || record.outcome === token) {
      score += 2
    }
  }
...
const actorIdFromPath = (path: Path): string => {
  for (const segment of path.split('/')) {
    if (segment.startsWith('actor_')) {
      return segment.slice('actor_'.length)
    }
  }
  return ''
}
