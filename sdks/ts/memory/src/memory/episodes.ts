...
export const createEpisodeRecorder = (deps: EpisodeRecorderDeps): EpisodeRecorder => {
  ...
  const get = async (sessionId: string): Promise<EpisodeRecord | undefined> => {
    ...
    try {
      const raw = (await deps.store.read(path)).toString('utf8')
      return parseEpisodeFile(path, raw)
    } catch {
      return undefined
    }
  }
  ...
  const list = async (opts: EpisodeListOptions = {}): Promise<readonly EpisodeRecord[]> => {
    ...
    return filtered.slice(0, normaliseLimit(opts.limit, DEFAULT_LIST_LIMIT))
  }
  ...
  const query = async (opts: EpisodeQueryOptions): Promise<readonly EpisodeQueryHit[]> => {
    ...
    const scored = base
      .map((episode) => ({ episode, score: scoreEpisodeQuery(episode, trimmedQuery, queryTokens) }))
    ...
    return scored.map(({ episode, score }) => ({ ...episode, score }))
  }
  ...
  const record = async (args: EpisodeRecordArgs): Promise<RecordEpisodeResult> => {
    ...
    const normalisedSummary = collapseWhitespace(args.reflection.summary)
    const recordName = `Episode ${args.sessionId}`
    const frontmatterTags = buildEpisodeTags({
      scope,
    ...
    await deps.store.batch({ reason: RECORD_REASON }, async (batch) => {
      ...
      if (await batch.exists(path)) {
        disposition = 'updated'
        const existingRaw = (await batch.read(path)).toString('utf8')
...
const buildEpisodeFile = (args: {
  ...
  if (args.payload.open_questions.length > 0) {
    ...
    for (const question of args.payload.open_questions) body.push(`- ${question}`)
    body.push('')
  }
...
const detectEpisodeSignals = (messages: readonly Message[]): EpisodeSignals => {
  ...
  for (const message of messages) {
    if (message.role !== 'system') substantiveMessageCount++
    if (message.role === 'user') userMessageCount++
    if (message.role === 'assistant') assistantMessageCount++
    if (message.role === 'tool') toolMessageCount++
    ...
    if (message.role === 'assistant') {
      for (const toolName of toolCallNames) {
        const normalisedToolName = toolName.trim().toLowerCase()
        if (WRITE_SIGNAL_TOOL_NAMES.has(normalisedToolName)) writeSignal = true
        if (EDIT_SIGNAL_TOOL_NAMES.has(normalisedToolName)) editSignal = true
      }
    }
  }
