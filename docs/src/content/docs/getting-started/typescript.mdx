...
Cross-SDK eval parity today is the shared daemon surface: `ask-basic`, `ask-augmented`, and `search-retrieve-only`. Native LongMemEval flows stay in the SDK-specific `memory eval lme` commands.
