---
title: Go
description: Install the Go SDK and run the implemented CLI surface.
sidebar:
  order: 2
---
...
For local checkout development, run `go build ./...` from the repository's
`go/` directory.
...
The production-ready Go CLI surface is `version`, `serve`, and `eval lme`.
Planned local workflow commands such as `init`, `ingest`, and `search` remain
hidden placeholders in the Go binary until they are implemented. See the [CLI
reference](/reference/cli/) for the cross-SDK command matrix.
...
Go ships the HTTP daemon, MCP wrapper, native LongMemEval runner, and public
packages. See the [Go SDK README](https://github.com/jeffs-brain/memory/tree/main/go) for the full surface.
