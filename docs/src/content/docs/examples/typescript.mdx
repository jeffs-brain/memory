...
3. Reads `docs/hedgehogs.md`, writes the raw content under `raw/documents/` via a batched `store.batch` call, then chunks the markdown with `chunkMarkdown` and upserts the chunks into the index.
